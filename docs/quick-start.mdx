...
- Team collaboration and suppressions
- Project policy managed in the dashboard
- GitHub-native features when you link a repo URL
...
Future uploads work automatically for that linked repo unless you pass `--no-upload`.

### Step 2: Know When You Need a Repo URL

Basic uploads do **not** require a GitHub repo binding.

Set the project repo URL later in `Dashboard -> Settings` when you want:
- GitHub Actions OIDC uploads
- GitHub App setup
- GitHub deep links and PR-native integration

### Step 3: Add More Scan Types

```bash
skylos . --danger --upload
skylos . --quality --upload
skylos . --danger --quality --upload
```

View results at: [skylos.dev/dashboard](https://skylos.dev/dashboard)

See [CLI to Dashboard Workflow](/cloud-workflow) for the full repo-to-cloud flow and [Authentication](/authentication) for CI and token details.
...
Use this when your Skylos project already has the correct GitHub `repo_url` saved in the dashboard.
...
### Option B: Token-Based GitHub Actions

Use this when you want explicit secret management or you have not set up OIDC yet.
...
- `skylos cicd init --upload` generates a **token-based** upload workflow
- `skylos sync setup` is also token-driven scaffolding, not the browser-based OIDC path

See [CI/CD Integration](/ci-cd) for the accurate OIDC vs token setup details.
...
| Scaffold token-based CI or hooks | `skylos cicd init --upload` or `skylos sync setup` |
| Connect to cloud | `skylos login` |
