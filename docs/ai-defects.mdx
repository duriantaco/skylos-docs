...
| Diff evidence | Unified git diff for changed files | Assertion weakened, CI permission added, CLI flag removed |
...
| `SKY-A103` | HIGH | GitHub Actions write permissions or privileged triggers added | Diff | Usually block |
| `SKY-A104` | MEDIUM | Public CLI flag removed from argparse, Click, or Typer surface | Diff | Warn unless compatibility policy says block |
...
CI permissions are not re-reported unless the PR newly adds them, and weak tests
are not reported unless the PR weakens them.
