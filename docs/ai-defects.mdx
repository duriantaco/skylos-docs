...
|---------|----------|---------------------------|
| `SKY-A101`, `SKY-A102` | `ai_defect` | New AI-defect-only rules |
...
| Diff evidence | Unified git diff for changed files | Assertion weakened in a test |
...
tests are not reported unless the PR weakens them, and high-risk source changes
only warn when the changed-file set has no accompanying test file.
