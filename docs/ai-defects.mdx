...
|---------|----------|---------------------------|
| `SKY-A101` to `SKY-A106` | `ai_defect` | New AI-defect-only rules |
...
| `SKY-A106` | LOW | Dependency version edit exactly repeats the project's own version change | Git snapshots and parsed Python packaging metadata | Review signal; configured gates still apply |
...
truth was enforced. See [AI Hallucination Contracts](./ai-contracts) for the
...
### Suspicious Dependency Version Bumps

`SKY-A106` looks for a release edit that changes a dependency from exactly the
same old version to exactly the same new version as the project itself. For
example, a project release from `3.4.1` to `3.4.2` may accidentally also change
`inquirer==3.4.1` to `inquirer==3.4.2` through a global text replacement.

```bash
# Compare committed PR changes with the merge base.
skylos . --ai-defects --diff-base origin/main --format json

# Compare local working files with HEAD.
skylos . --ai-defects --format json
```

This is separate from checking whether a package or version exists. A real
dependency version can still be an unintended change. The rule is a LOW review
signal, makes no registry requests, and never runs packaging code. It currently
supports literal Python project metadata, requirements files, and uv/Poetry
package version records. See the [rule reference](./rules-reference#sky-a106--suspicious-dependency-version-bump)
for supported declarations, comparison behavior, ignores and gate semantics.
