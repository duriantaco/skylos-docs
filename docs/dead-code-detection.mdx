...
description: "Detect unused functions, imports, classes, and variables in Python, TypeScript, JavaScript, Java, Go, Kotlin, PHP, Rust, Dart, and C#. Framework-aware dead code detection for Django, FastAPI, Flask, Spring, React, Next.js, Flutter, and Pydantic."
---
...
For proprietary frameworks, plugin loaders, and decorator registries, add
project-specific entrypoint rules instead of broad whitelists:
...
dead code in the same files. See [Configuration](/configuration) for every
...
```
  ...
  <TabItem value="concise" label="Concise">
```bash
