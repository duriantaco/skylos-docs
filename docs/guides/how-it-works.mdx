...
## Security Taskflow (`skylos agent scan . --security`)

The static pipeline above powers the baseline scan. The security-agent path adds a repo-aware taskflow on top of it:

```mermaid
flowchart LR
    A[Repo activation index] --> B[repo_map]
    B --> C[Framework-aware file facts]
    C --> D[Whole-file security audit]
    D --> E[Verifier re-review]
    E --> F[Final findings]
...
`repo_map` derives repo context, entrypoint hints, trust boundaries, and per-file facts such as:

- detected framework (`flask`, `fastapi`, `django`)
- user-controlled sources
- dangerous sinks
- guards and sanitizers

The taskflow then runs whole-file security review, re-verifies surviving findings, and keeps a candidate ledger so evidence can be tracked across `audit -> verify -> finalize`.
...
- `rules/ai_defect/` — Evidence-backed AI-code hallucination checks
