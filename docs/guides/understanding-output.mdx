...
In JSON output, strict hallucination checks are grouped under `ai_defects`:
...
This bucket currently includes `SKY-L012`, `SKY-L023`, `SKY-D222`, `SKY-D224`,
and `SKY-D225`. `SKY-D223` remains a security/supply-chain finding because an
undeclared but real dependency is not a hallucination.
