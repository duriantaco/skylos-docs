...
This bucket currently includes `SKY-A101`, `SKY-A102`, `SKY-L012`, `SKY-L023`,
`SKY-D222`, `SKY-D224`, and `SKY-D225`. Grouping is category-based, not only
prefix-based: historical `SKY-L` and `SKY-D` IDs are kept for compatibility
when their catalog category is `ai_defect`.
