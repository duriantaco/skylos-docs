...
Use `skylos clean` to preview safe import cleanup without editing files:

**skylos:**


```bash
skylos clean . --dry-run --types imports --confidence 90
```

The output is grouped by file and line:

```
Dry run: would remove 3 dead code items.

api/views.py
  L3 import json (95%)
models.py
  L1 import Optional (92%)
utils/helpers.py
  L5 import deprecated_lib (90%)
```

### Step 3: Apply Changes

```bash
# Remove matching imports without prompting
skylos clean . --apply --types imports --confidence 90
```
...
```bash
skylos . --confidence 80 | grep -A 100 "Unreachable Functions"
```
...
```bash
skylos clean . --apply --comment-out --types function --confidence 80
```
...
```bash
skylos . --confidence 50 | grep -A 50 "Unreachable"
```
