...
| TypeScript type safety | TypeScript strict mode | `SKY-T103`, `SKY-T104`, `SKY-T105`, `SKY-R105`, advisory TypeScript build |
...
| Maintainability, reliability, and complexity | ISO/IEC 5055, ISO/IEC 25010, CWE coding-standard classes | `SKY-Q*`, `SKY-C*`, `SKY-L*`, `SKY-P*` |
...
| `SKY-T103` | Chained TypeScript assertions through `any`, `unknown`, `object`, or `{}` | Broad bridge types bypass evidence that a precise conversion is safe |
| `SKY-T104` | Effective `@ts-ignore` or file-wide `@ts-nocheck` | Compiler errors can be hidden instead of fixed or reviewed narrowly |
| `SKY-T105` | Direct assertions from `JSON.parse()` or proven fetch JSON results | Type assertions do not validate runtime input |
