---
title: "Configuration"
description: "Configure Skylos via pyproject.toml. Set exclusions, confidence thresholds, prompt templates, vibe dictionaries, quality gates, and framework-specific settings."
---
...
# Optional prompt extensions for LLM-powered workflows.
# These extend Skylos' built-in prompts; they do not replace JSON output or untrusted-code safety rules.
...
[tool.skylos.vibe]
# Optional dictionary extensions for project-specific vibe-code checks.
extra_phantom_names = []
...
| `templates` | table | built-in prompts | Optional prompt-template extensions for LLM-powered security, quality, review, and audit workflows |
| `vibe` | table | built-in dictionaries | Optional dictionary extensions for phantom helpers, decorators, credential names, sensitive files, and network timeout calls |

### Prompt Templates And Vibe Dictionaries

Use `[tool.skylos.templates]` when you want LLM-powered workflows to include project-specific review instructions. Template files are appended to Skylos' built-in system prompts, while Skylos keeps its JSON-only output contract and untrusted-input safety framing.

Use `[tool.skylos.vibe]` when your project has local names Skylos should treat as suspicious. This is useful for fake auth helpers, house credential names, sensitive file naming conventions, and SDK calls that must set timeouts.
