...
modes: `SKY-L012`, `SKY-L023`, `SKY-D222`, `SKY-D224`, and `SKY-D225`.
...
Strict hallucination checks report under the `ai_defects` output bucket, even
when they retain historical `SKY-L` or `SKY-D` rule IDs.
