...
Rule IDs are **unified across languages** — when multiple scanners detect the same vulnerability class, Skylos reports it with the same ID across Python, TypeScript/JavaScript, Java, Go, PHP, Rust, Dart, Shell, and other supported ecosystems.
...
| Rule ID | Name | Severity | Languages | Description |
|---------|------|----------|-----------|-------------|
| `SKY-D211` | SQL Injection | CRITICAL | Python, TS, Java, Go | Tainted/interpolated SQL in `execute`, template literals, `PreparedStatement`, or `fmt.Sprintf` |
| `SKY-D212` | Command Injection | CRITICAL | Python, TS/JS, Java, Go, Rust, Dart, Shell | Tainted input to `os.system()`, `child_process.exec()`, `Runtime.exec`, `exec.Command`, Rust `Command::new`, Dart `Process.*`, shell `eval`, `source`, or shell `-c` |
| `SKY-D215` | Path Traversal | HIGH | Python, Java, Go, PHP, Rust, Dart, Shell | Tainted input used in filesystem paths, include/file sinks, shell file commands, or redirection targets |
| `SKY-D216` | SSRF | CRITICAL | Python, TS, Java, Go, Dart, Shell | Tainted URL passed to HTTP clients, request builders, `curl`, or `wget` |
...
| `SKY-D224` | API Signature Hallucination | HIGH | Python | Call site uses a real installed package API that does not exist or does not accept the supplied keyword |
| `SKY-D225` | Dependency Version Hallucination | HIGH | npm, Go | Manifest pins a package version that does not exist in the package registry |
