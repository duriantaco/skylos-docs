...
description: "Complete reference for all Skylos static analysis rules — security vulnerabilities, vibe coding guardrails, prompt injection detection, code quality, and dead code."
---
...
Rule IDs are **unified across languages** — when multiple scanners detect the same vulnerability class, Skylos reports it with the same ID across Python, TypeScript/JavaScript, Go, PHP, Rust, and other supported ecosystems.
...
| `SKY-D212` | Command Injection | CRITICAL | Python, TS/JS, Go, Rust | Tainted input to `os.system()`, `child_process.exec()`, `exec.Command`, or Rust `Command::new` / shell `-c` |
| `SKY-D215` | Path Traversal | HIGH | Python, Go, PHP, Rust | Tainted input used in filesystem paths or include/file sinks |
| `SKY-D216` | SSRF | CRITICAL | Python, TS, Go, Java | Tainted URL passed to HTTP clients or request builders |
...
| `SKY-D230` | Open Redirect | HIGH | Python, TS, Go, Java | User-controlled URL in `redirect()` / `res.redirect()` / `http.Redirect` / `sendRedirect` |
...
`action.yml`, and `action.yaml` when `--danger` is enabled.
...
| `SKY-D291` | Broad GitHub Token Permissions | MEDIUM-HIGH | Missing explicit permissions, `write-all`, or workflow-level write scopes |
...
| `SKY-D294` | GitHub Actions Template Injection | HIGH | Attacker-controlled GitHub context is expanded in `run`, code-like action inputs, or container/service options |
| `SKY-D295` | Self-Hosted Or Dynamic Runner | MEDIUM | Job uses self-hosted runners, runner groups, or expression-derived runner labels |
| `SKY-D296` | Mutable Container Image | MEDIUM-HIGH | Job containers, service containers, Docker actions, `docker pull`, or `docker run` use unpinned images |
| `SKY-D297` | Reusable Workflow Secrets Inherit | MEDIUM | Reusable workflow receives all caller secrets via `secrets: inherit` |
| `SKY-D298` | Overbroad Secrets Context | MEDIUM | Workflow expands `toJSON(secrets)` or indexes `secrets[...]` dynamically |
| `SKY-D299` | Secret Without Environment Boundary | MEDIUM | Job uses repository secrets without an `environment` boundary |
...
| `SKY-D303` | Unsound `contains()` Authorization | HIGH | `contains('literal', user_controlled_context)` can be bypassed |
| `SKY-D304` | Spoofable Bot Actor Check | HIGH | Workflow trusts bot-like actor strings as an authorization boundary |
| `SKY-D305` | Multiline `if` Truthiness Footgun | HIGH | Fenced `if: |` or `if: >` expression can evaluate as a truthy string |
...
| `SKY-D308` | Release Cache Poisoning Risk | MEDIUM | Release-like workflow restores mutable cache state before publishing artifacts |
| `SKY-D309` | Broad Secret Environment Scope | MEDIUM-HIGH | Secret is exposed at workflow or job `env` scope instead of the specific step |
...
| `SKY-D311` | Lax Artifact Upload Policy | LOW | `actions/upload-artifact` omits `if-no-files-found: error` |
| `SKY-D312` | JavaScript Install Scripts In CI | MEDIUM | `npm`, `pnpm`, `yarn`, or `bun` install runs lifecycle scripts without `--ignore-scripts` |
| `SKY-D313` | Missing Privileged Job Timeout | LOW | Privileged or release-like job has no `timeout-minutes` |
