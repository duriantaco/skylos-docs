...
Some rules keep their historical `SKY-L` or `SKY-D` IDs but report under the
`ai_defects` output bucket because they prove AI-code hallucination failure
modes: `SKY-L012`, `SKY-L023`, `SKY-D222`, `SKY-D224`, and `SKY-D225`. New
AI-defect rules use the `SKY-A` prefix.
...
AI-defect checks report under the `ai_defects` output bucket. Some retain
historical `SKY-L` or `SKY-D` rule IDs; newer checks use `SKY-A`.

| Rule ID | Name | Severity | Languages | Description |
|---------|------|----------|-----------|-------------|
| `SKY-A101` | Test Assertion Weakening | MEDIUM | Diff-aware tests | Specific or exception assertion replaced with a broad truthiness/null check, skip, or xfail |
