...
| `SKY-A103` | CI Permission Expansion | HIGH | GitHub Actions | Workflow diff adds write permissions or privileged triggers |
...
| **What** | A workflow diff adds `permissions: write-all`, a write-scoped GitHub token permission, `pull_request_target`, or `workflow_run` |
| **Risk** | AI-authored CI changes can accidentally expand token privileges or run privileged workflows against untrusted code |
| **Output Bucket** | `ai_defects` |
...
Skylos reports this from the diff only. It ignores unchanged moved lines, so an
existing broad permission is not re-reported unless the PR newly adds or changes
...
| **What** | A public CLI flag is removed from argparse, Click, or Typer code without the same flag being re-added in the diff |
| **Risk** | AI-generated refactors can silently break scripts, CI jobs, docs, and users that depend on stable command-line flags |
| **Output Bucket** | `ai_defects` |
...
Skylos treats this as a compatibility review signal. It does not say the removal
is wrong; it asks the reviewer to confirm the breaking change, keep an alias, or
