...
description: "Complete reference for all Skylos static analysis rules — security vulnerabilities, AI defects, vibe coding guardrails, prompt injection detection, code quality, and dead code."
---
...
| `SKY-DEP` | Deployment Security & Reliability | Correlated Kubernetes exposure and unsafe runtime-mode findings |
| `SKY-GPU` | GPU Release Reliability | Fail-closed CUDA, build-architecture, and TensorRT compatibility findings |
...
| `SKY-D254` | HTTP Session Trust Boundary Violation | HIGH | Java | Servlet-controlled data crosses into HTTP session state (CWE-501) |
...
| `SKY-D282` | Webhook Signature Verification Issue | HIGH | Python, TS | Matching request payload is not proven verified before body trust or event side effects |
...
| `SKY-D280` | Next.js Mutating Route Missing Authentication | HIGH | Protected mutation is reachable without a proven route-local rejecting authentication guard |
| `SKY-D281` | Next.js Server Action SQL Injection | CRITICAL | Untrusted Server Action input reaches SQL text without a parameterized binding proof (CWE-89) |
...
checks use `SKY-A`. See [AI Defect Verification](./ai-defects) for the method
and output contract.
...
| `SKY-A106` | Suspicious Dependency Version Bump | LOW | Python manifests and lock files in Git changes | A dependency's old and new versions exactly match the project's own version change |
...
| `SKY-D305` | Multiline `if` Truthiness Footgun | MEDIUM | Fenced `if: |` or `if: >` expression can evaluate as a truthy string |
...
These opt-in rules prove an exposure path across one rendered,
multi-document Kubernetes file: an explicitly external, plain-HTTP Ingress
must resolve through a Service and workload to one container. Skylos abstains
when an edge in that chain is missing, dynamic, or ambiguous.

The Ingress must declare `skylos.dev/network-scope: external` (or `public`) and
`skylos.dev/backend-protocol: http`. `SKY-DEP001` also requires the workload
Pod template to declare `skylos.dev/source-file` and
`skylos.dev/required-guards`.

| Rule ID | Name | Severity | Category | Description |
|---------|------|----------|----------|-------------|
| `SKY-DEP001` | Externally Deployed Route Missing Required Auth Guard | HIGH | Security | An Ingress-reachable sensitive FastAPI or Flask route is missing a guard required by the workload contract |
| `SKY-DEP002` | External Ingress Exposes Flask Development Debugger | HIGH | Security | The resolved externally routed Flask container effectively enables its development debugger |
| `SKY-DEP003` | External Ingress Routes To Reload-Mode Server | MEDIUM | Reliability | The resolved Flask, Uvicorn, or Gunicorn container effectively enables reload mode |

```bash
# Discover this rule family
skylos rules list deployment

# Scan only the deployment proof rules
skylos . --select SKY-DEP001,SKY-DEP002,SKY-DEP003 --format concise
```

These are correlated static proofs, not live-cluster checks. Skylos does not
render Helm or Kustomize, join resources split across files, or infer network
policy. See [Release Reliability](./release-reliability#kubernetes-deployment-exposure-proof)
for the contract and proof chain.

### GPU Release Compatibility (SKY-GPU)

Skylos treats `.skylos/gpu-targets.yml` (or `.yaml`) version 1 as the release
target contract and cross-checks it against effective final-stage Docker CUDA
images, CMake/NVCC architecture output, and packaged TensorRT engines. It does
not probe installed hardware or perform package CVE analysis.
...
The contract is fail-closed once present or explicitly selected: invalid,
deleted, ambiguous, or incomplete contract/evidence state produces
`SKY-GPU000` instead of a confident compatibility result.

| Rule ID | Name | Severity | Category | Description |
|---------|------|----------|----------|-------------|
| `SKY-GPU000` | GPU Release Contract Is Invalid Or Incomplete | HIGH | Reliability | Contract validation or bounded evidence discovery could not complete |
| `SKY-GPU001` | CUDA Image Requires Newer NVIDIA Driver | HIGH | Reliability | The effective final CUDA image requires a newer driver branch than a declared target provides |
| `SKY-GPU002` | CUDA Target Architecture Missing From Build | HIGH | Reliability | Emitted SASS/PTX does not cover a declared target compute capability |
| `SKY-GPU003` | TensorRT Engine Is Not Portable Across Target GPUs | HIGH | Reliability | A packaged serialized TensorRT engine lacks the compatibility evidence required by the declared target fleet |

```bash
# Discover this rule family
skylos rules list gpu

# Fail the release gate on an invalid or incompatible GPU artifact
skylos . --select SKY-GPU000,SKY-GPU001,SKY-GPU002,SKY-GPU003 --gate --format concise
```

Selecting `SKY-GPU001`, `SKY-GPU002`, or `SKY-GPU003` also retains
`SKY-GPU000`, so a missing or invalid required contract cannot disappear behind
an exact-rule filter. See [Release Reliability](./release-reliability#gpu-release-compatibility)
for setup and evidence semantics.
...
| `SKY-S102` | Client-Side Secret Exposure | HIGH–CRITICAL | TS/JS, HTML, client bundles | Secret material or a server-only environment variable reaches client-accessible code |
...
| `SKY-Q405` | Async Promise Executor | HIGH | TypeScript, JavaScript | `new Promise(async ...)` ignores the executor's async result |
| `SKY-Q406` | Async Array.forEach Callback | HIGH | TypeScript, JavaScript | Built-in `Array.forEach` does not wait for async callbacks |
| `SKY-Q407` | Discarded Async Array.map Result | HIGH | TypeScript, JavaScript | Promises returned by `Array.map(async ...)` are discarded |
...
| `SKY-T103` | Suspicious Chained Type Assertion | MEDIUM | TypeScript | Assertion reaches a precise type through `any`, `unknown`, `object`, or `{}` |
| `SKY-T104` | TypeScript Compiler Suppression Directive | MEDIUM–HIGH | TypeScript, JavaScript | Effective `@ts-ignore` or file-wide `@ts-nocheck` hides compiler errors |
| `SKY-T105` | Unvalidated JSON Type Assertion | MEDIUM | TypeScript | Direct `JSON.parse()` or proven fetch `Response.json()` result is asserted without runtime validation |
| `SKY-T106` | Unsafe Exported API Type | MEDIUM | TypeScript | Exported API uses exact `any`, `Record<string, any>`, or an any-valued index signature |
...
| `SKY-L007` | Empty Error Handler | MEDIUM–HIGH | Python handlers or TS/JS `catch` blocks silently swallow errors |
...
| `SKY-L026` | Unfinished code or placeholder default | MEDIUM | Python placeholder body/default or TS/JS function that only throws a not-implemented marker |
...
| `SKY-L034` | Repeated Mutable Alias | MEDIUM | Sequence multiplication reuses mutable elements across repetitions |
| `SKY-L035` | Blanket ESLint Disable | HIGH | A leading bare `/* eslint-disable */` turns off every ESLint rule for the file |
...
|---|---|
| **Severity** | LOW |
| **Languages** | Python project manifests, requirements files, uv and Poetry lock files |
| **What** | A dependency version edit exactly repeats the project's own old and new versions |
| **Risk** | A global version replacement can accidentally change an unrelated dependency during a release |
| **Output Bucket** | `ai_defects` |
...
The matching version change is a reason to review the edit, not proof that the
dependency version is invalid. Unlike `SKY-D225`, this rule does not query a
registry. It does not execute `setup.py` or change dependencies.

For committed PR changes:

```bash
skylos . --ai-defects --diff-base origin/main --format json
```

Without a diff option, the rule compares local working files with HEAD. With
`--diff-base`, it compares the merge base with committed HEAD, leaving
uncommitted edits out of that comparison. `--diff origin/main` uses the same
committed comparison and filters findings to changed lines. When both options
are supplied, `--diff-base` chooses the comparison base.

The `skylos agent pre-commit` command compares HEAD with staged Git files, so
unstaged edits do not change the dependency versions being checked.

The initial scope is literal project names and versions in `pyproject.toml` or
static setuptools/distutils `setup.py` calls. Dependency declarations include
project, build, development and optional dependency groups, requirements files,
and actual package version records in `uv.lock` and `poetry.lock`. Version
exclusions such as `!=3.4.1` also count. Dynamic project versions, `setup.cfg`,
other manifest formats and ambiguous changes are not inferred.

Findings identify the dependency declaration and carry the project name,
dependency name, both version pairs and project source location in `metadata`.
They use `signal_only: true` and `blocking_recommended: false`. These fields
recommend review; they do not override configured gate thresholds or `--strict`.
The existing precommit gate blocks reported AI findings, including this advisory.
Likewise, `verify_change` keeps the finding and returns its existing `fail` status
when findings are present; it does not treat advisory metadata as an exemption.

This advisory uses project ignores rather than inline comments. To disable it,
add the rule to the existing ignore list:
...
Skylos validates package/version existence for `package.json` and `go.mod` manifests. In `package.json`, Skylos only checks dependency sections such as `dependencies`, `devDependencies`, `peerDependencies`, and `optionalDependencies`, not arbitrary config objects. Registry lookups are constrained to fixed npm and Go hosts, package coordinates are validated before request construction, and responses are size-capped.
...
| **What** | An HTTP response cookie is missing a proven `httpOnly: true` or `secure: true` setting |
| **Risk** | Script access or cleartext transport can expose session credentials |

For TypeScript and JavaScript, Skylos follows response-object aliases and evaluates cookie options as `true`, `false`, absent, or unknown. Only proven `true` for both flags establishes the safe case; explicit `false`, missing flags, and unresolved runtime values remain findings.

```typescript
// Bad
response.cookie("session", token, { httpOnly: true, secure: false });

// Good
response.cookie("session", token, { httpOnly: true, secure: true });
```

---

### SKY-D280 · Next.js Mutating Route Missing Authentication

| | |
|---|---|
| **Severity** | HIGH |
| **Languages** | TypeScript / JavaScript |
| **What** | A mutating Next.js route reaches a protected side effect without a proven route-local authentication guard |
| **Risk** | Unauthenticated callers may create, update, or delete protected data |

Skylos requires a trusted authentication result and a rejecting guard that dominates the mutation. Auth-shaped comments, imports, headers, cookies, or unused helper calls do not count as protection. Recognized framework helpers include Auth.js/NextAuth and Clerk patterns when their result actually controls the route.
...
| **Severity** | CRITICAL |
| **Languages** | TypeScript / JavaScript |
| **What** | Untrusted Server Action input reaches executable SQL text without a parameterized-query proof |
| **Risk** | Attackers may read, modify, or delete database data (CWE-89) |

The bounded proof engine follows Server Action parameters through local helpers, classes, closures, object properties, control flow, exceptions, async/`await`, and native Promise reactions. It distinguishes SQL text from bound values, recognizes proven parameterizing APIs and tagged templates, and reports incomplete analysis instead of silently declaring safety when a proof budget is exhausted.
...
"use server";
export async function find(input: string) {
  return db.query("SELECT * FROM users WHERE name = $1", [input]);
}
```
...
| **What** | The matching request payload is not proven signature-verified before body trust or event side effects |
| **Risk** | Forged webhook requests can trigger billing, account, or workflow state changes |

Skylos reports this only when the file looks like a real inbound webhook handler. A verifier must consume the matching raw request payload and provider signature before parsed-body trust or event side effects. Verification of an unrelated value, a verifier call on only one non-dominating branch, or a check after the side effect does not establish safety.
...
| | |
|---|---|
| **Severity** | HIGH for server-only environment references; CRITICAL for a concrete hardcoded credential |
| **Languages** | TypeScript / JavaScript, HTML, and client bundle artifacts |
| **What** | Secret material or a server-only environment variable reaches code delivered to a browser/client |
| **Risk** | Anyone loading or inspecting the client bundle can recover the value |

```typescript
// Bad
"use client";
const billingToken = process.env.STRIPE_SECRET_KEY;

// Good
// Keep the secret server-side and expose only non-sensitive public config.
const publicRegion = process.env.NEXT_PUBLIC_REGION;
```

Skylos also scans generated client artifacts for concrete credential patterns. When a client exposure is the same concrete secret originally detected by `SKY-S101`, it is emitted as `SKY-S102` while retaining CRITICAL severity.
...
### SKY-T103 · Suspicious Chained Type Assertion

| | |
|---|---|
| **Severity** | MEDIUM |
| **Languages** | TypeScript |
| **What** | A chained assertion uses `any`, `unknown`, `object`, or `{}` to force a value into a more precise type |
| **Risk** | The broad middle type bypasses TypeScript's evidence that the conversion is safe |

```typescript
// Flagged: the unknown bridge forces input to User
const user = input as unknown as User;

// Better: narrow or validate the value first
const user = UserSchema.parse(input);
```

Normal single assertions and chains between precise types are not reported.
...
| | |
|---|---|
| **Severity** | MEDIUM for `@ts-ignore`; HIGH for effective `@ts-nocheck` |
| **Languages** | TypeScript, JavaScript |
| **What** | A compiler directive hides errors on one line or disables checking for the whole file |
| **Risk** | Real type regressions can pass builds without review |
...
// Better for a narrow, intentional exception
// @ts-expect-error -- upstream types reject this supported value
configureLegacyOption("supported-value");
```

Skylos follows TypeScript's leading file pragma behavior: the last leading
`@ts-check` or `@ts-nocheck` decides whether file checking is enabled.
`@ts-expect-error` is not reported because it fails when the expected error
disappears.
...
| **Severity** | MEDIUM |
| **Languages** | TypeScript |
| **What** | Dynamic `JSON.parse()` data or a proven fetch `Response.json()` result is asserted directly as a domain type |
| **Risk** | Type assertions do not validate runtime data, so malformed input can enter typed code unchecked |
...
const user = UserSchema.parse(JSON.parse(payload));
const account = AccountSchema.parse(await response.json());
```

The rule only reports direct, precise assertions when Skylos can prove the
built-in JSON or fetch boundary. It skips shadowed or replaced built-ins,
mutated response methods, generated files, declaration files, and ambiguous
provenance.
...
| **Severity** | MEDIUM |
| **Languages** | TypeScript |
| **What** | An exported API uses exact `any`, built-in `Record<string, any>`, or an index signature whose value is `any` |
| **Risk** | Callers lose useful type checking at the public boundary |

```typescript
// Flagged
export function parse(value: any): Record<string, any> {
  return value;
}
...
The rule checks types that callers can see: function parameters and returns,
overloads, interfaces, public class members, generic defaults, tuple entries,
export assignments, destructured exports, and simple exported objects.
Type-only class and namespace exports only check their type side. An explicit
type always wins over an inferred value or default.

Skylos does not guess when it cannot tell which local value an export uses. It
also skips nested uses such as `Promise<any>`, private or protected members,
non-built-in `Record` types, generated files, and malformed syntax.
...
| **Severity** | HIGH |
| **Languages** | TypeScript, JavaScript |
| **What** | `new Promise(async (...) => ...)` passes an async executor to the Promise constructor |
| **Risk** | The constructor ignores the executor's async result. An async-generator executor does not run, so the outer promise can stay pending |

```typescript
// Flagged
const value = new Promise(async (resolve) => {
...
The rule requires the built-in `Promise` binding and a direct inline async
executor. It also catches async-generator executors, whose body never runs.
Shadowed or replaced constructors are not reported.
...
| **Severity** | HIGH |
| **Languages** | TypeScript, JavaScript |
| **What** | A proven built-in array passes an async callback to `forEach` |
| **Risk** | `forEach` does not wait for callback promises, so the caller can continue before work finishes |
...
The callback can be inline or a stable local async function. Skylos only
reports receivers it can prove are built-in arrays. Custom `forEach` methods
and ambiguous or mutated bindings are skipped.
...
| **Severity** | HIGH |
| **Languages** | TypeScript, JavaScript |
| **What** | Promises from a proven array `map` call with an async callback are discarded |
| **Risk** | The mapped promises are not consumed, so work can outlive the caller and failures can go unhandled |

```typescript
// Flagged
items.map(async (item) => save(item));

// Better: concurrent and awaited
await Promise.all(items.map(async (item) => save(item)));
```

The callback can be inline or a stable local async function. Assigning,
returning, or passing the mapped promises to another call is not reported.
Directly awaiting the array is still reported because `await` does not wait
for the promises stored inside it. Skylos also reports code that only reads
the mapped array's length, or drops a built-in `.filter(Boolean)` chain.
...
An empty TS/JS catch can be left unreported by adding a clear comment that
explains why ignoring that specific error is safe.

Skylos skips this check in test, fixture, mock, story, E2E, and Cypress files.
...
### SKY-L026 · Unfinished code or placeholder default

| | |
|---|---|
| **Severity** | MEDIUM |
| **Languages** | Python, TypeScript, JavaScript |
| **What** | Python placeholder body/default or TS/JS function that only throws a static not-implemented marker in production code |
...
```typescript
// Flagged
function sendNotification(): never {
  throw new Error("Not implemented");
}
```

Skips abstract methods (`@abstractmethod`), declarations, test files,
`__init__.py`, dunder methods (`__repr__`, `__eq__`, etc.), generated files,
and functions that do real work before throwing.
...
| **Severity** | MEDIUM |
| **Languages** | Python |
| **What** | Sequence multiplication repeats mutable elements by reference instead of creating independent values |
| **Risk** | Updating one entry can unexpectedly change other entries that share the same object |

```python
# Flagged: every row is the same list
matrix = [[0] * columns] * rows

# Better: each row is independent
matrix = [[0] * columns for _ in range(rows)]
```

The rule reports mutable literals and comprehensions, including nested mutable
values inside tuples. It skips counts that cannot repeat more than once and
ambiguous named objects whose mutability cannot be proved.
...
| **Severity** | HIGH |
| **Languages** | TypeScript, JavaScript |
| **What** | A leading bare `/* eslint-disable */` turns off every ESLint rule for the file |
| **Risk** | Type, correctness, and security lint checks can all disappear without a visible build failure |
...
// Better: name the one required rule
/* eslint-disable no-console */
console.log("legacy output");
```

A later bare `/* eslint-enable */` closes the suppression and is not reported
as file-wide. JSDoc comments are not treated as ESLint directives.
