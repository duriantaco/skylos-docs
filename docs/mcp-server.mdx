...
Analysis tools (`analyze`, `security_scan`, `quality_check`, `secrets_scan`) run entirely on your machine — they don't cost credits. Only the `remediate` tool, which uses LLM-powered fix generation, costs 10 credits per call.
