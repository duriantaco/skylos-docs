...
| **AI Defects** | Hallucinated dependencies, real packages called with invented APIs, npm/Go versions that do not exist, and weakened assertions | SKY-A101, SKY-D222, SKY-D224, SKY-D225 |
| **Supply Chain** | Undeclared but real third-party imports | SKY-D223 |
...
**Returns**: Security danger findings with file, line, severity, and description. AI-defect checks such as hallucinated dependencies and invented package APIs are reported through `verify_change` and the `ai_defects` output bucket, not the danger bucket.
...
hallucinations, API signature hallucinations, and weakened assertions. AI-defect
findings use `category: "ai_defect"`.
