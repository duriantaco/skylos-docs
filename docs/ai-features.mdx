...
Security-focused scan to catch security vulnerabilities:

```bash
skylos agent scan --security ./src
```
...
:::note
For best results, use `skylos agent scan` and `skylos agent scan --security` instead. They provide full project context to the LLM, enabling detection of cross-file issues and hallucinated function calls.
:::
