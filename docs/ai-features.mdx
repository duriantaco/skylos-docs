...
list, see [AI Defect Verification](./ai-defects).
...
`confidence`, `severity`, and `category` for each finding. Without `--no-fail`,
the command exits `1` when the verifier returns a failing status.
...
`line_range`, `project_context`, `confidence`, and
