...
Strict hallucination findings use `category: "ai_defect"` and full scans group
them under the `ai_defects` output bucket.
...
Strict hallucination checks (`SKY-L012`, `SKY-L023`, `SKY-D222`, `SKY-D224`,
and `SKY-D225`) report under the `ai_defects` output bucket; broader AI-prone
quality and security rules keep their existing buckets.
