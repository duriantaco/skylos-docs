...
Verify a changed file, range, or stdin code blob for AI-code trust findings. Add
...
```bash
# Gate only the GPU compatibility proof
...
See [Release Reliability](./release-reliability) for Kubernetes exposure proof
and GPU artifact compatibility.
...
| Code | Meaning |
|------|---------|
| `0` | The command completed and its selected output, report, or policy passed |
...
[Scanner Comparison](./scanner-comparison#exit-codes).
